...
We are going to remove some of the data.  In particular, any row that has an `NA` for the `VALUE` field.
...
When we push data into the database, we do the same thing each time.  We can wrap this in a function:
...
Multi-factor (Employment &cetera.)
